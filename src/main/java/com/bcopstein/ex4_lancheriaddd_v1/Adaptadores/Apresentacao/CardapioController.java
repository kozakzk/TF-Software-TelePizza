...
import com.bcopstein.ex4_lancheriaddd_v1.Aplicacao.AtivaCardapioUC;
...
public class CardapioController {
    ...
    private AtivaCardapioUC ativaCardapioUC;
    ...
    public CardapioController(RecuperarCardapioUC recuperaCardapioUC,
            ...
            AtivaCardapioUC ativaCardapioUC,
            RecuperaCardapioAtivoUC recuperaCardapioAtivoUC) {
        this.recuperaCardapioUC = recuperaCardapioUC;
        this.recuperaListaCardapioUC = recuperaListaCardapioUC;
        this.ativaCardapioUC = ativaCardapioUC;
    ...
    public CardapioPresenter recuperaCardapioAtivo() {
        CardapioResponse cardapioResponse = recuperaCardapioAtivoUC.run();
        if (cardapioResponse.getCardapio() == null) {
            return null;
        }
        Set<Long> conjIdSugestoes = new HashSet<>(cardapioResponse.getSugestoesDoChef().stream()
                .map(produto -> produto.getId())
    ...
    @CrossOrigin("*")
    public boolean ativaCardapio(@PathVariable(value = "id") long id) {
        return ativaCardapioUC.run(id);
    }
}
